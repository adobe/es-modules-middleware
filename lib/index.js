 ...
 * @param {string} requestedPath - the requested path to serve
 */
const serveResponse = (res, next, paths, requestedPath) => {
...
const EsModulesMiddlewareFactory = function(config) {
    ...
    return function(req, res, next) {
        ...
        // strip off the /base prefix, but leave the preceeding /
        const requestedPath = req.originalUrl.slice('/base'.length);
        return serveResponse(res, next, config.paths, requestedPath);
    };
};
 ...
 *
 * @param {Object} paths - a map of path prefix to filesystem path from which to serve files.
 * @returns {Function} - returns the middleware which will resolve the paths based on the given paths map.
 */
function middleware(paths) {
    return function(req, res, next) {
        return serveResponse(res, next, paths, req.originalUrl);
    };
}
module.exports = {
